...
app = FastAPI(
    title="Rule 104 — FIND Requires MODE",
    version="2.0"
)
...
class Finding(BaseModel):
    ...
    issues_type: Optional[str] = None    # FindWithoutMode
...
# ---------------------------------------------------------------------------
# 1) Capture ONE ABAP statement that starts with FIND and ends at the period.
STMT_RE = re.compile(r"(?is)\bFIND\b[^.]*\.", re.DOTALL)

# 2) Inside the statement, check whether MODE is explicitly present.
...
def scan_unit(unit: Unit) -> Unit:
    ...
    for m in STMT_RE.finditer(src):
        ...
        if not has_mode:
            ...
            # Snippet = full line containing the FIND statement
            ...
            msg = "FIND without MODE. Add IN CHARACTER MODE for text or IN BYTE MODE for binary."
            sug = (
                "FIND <pat> IN <text> IN CHARACTER MODE.\n"
                "OR\n"
                "FIND <xpat> IN <xbin> IN BYTE MODE."
            )
...
@app.get("/health")
async def health():
    return {"ok": True, "rule": 104, "version": "2.0"}
